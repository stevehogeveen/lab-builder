...
async def save_config_handler(
    ...
    cfg = {
        ...
        "cisco_switch": {
            ...
            "console_password": preserve_existing_secret(cisco_console_password, (existing_cfg.get("cisco_switch") or {}).get("console_password")),
            ...
            "management_port": cisco_management_port or str(((existing_cfg.get("cisco_switch") or {}).get("management_port")) or "GigabitEthernet1/0/1"),
            "management_port_mode": cisco_management_port_mode or str(((existing_cfg.get("cisco_switch") or {}).get("management_port_mode")) or "do_not_touch"),
            "trusted_console_adapter": (str(cisco_trusted_console_adapter).lower() in {"1", "true", "yes", "on"}) if cisco_trusted_console_adapter is not None else bool((existing_cfg.get("cisco_switch") or {}).get("trusted_console_adapter")),
            ...
            "enable_password": preserve_existing_secret(cisco_enable_secret or cisco_enable_password, (existing_cfg.get("cisco_switch") or {}).get("enable_secret") or (existing_cfg.get("cisco_switch") or {}).get("enable_password")),
            "domain_name": cisco_domain_name or str(((existing_cfg.get("cisco_switch") or {}).get("domain_name")) or "lab.local"),
...
async def save_global_settings_handler(
    ...
    cisco_fields = {
        ...
        "cisco_management_port",
        "cisco_management_port_mode",
        "cisco_trusted_console_adapter",
    }
    if any(field in form for field in cisco_fields):
        existing_cisco = cfg.get("cisco_switch") or {}
        submitted_enable_secret = cisco_enable_secret if cisco_enable_secret is not None else cisco_enable_password
        cfg["cisco_switch"].update(
            {
                ...
                "console_password": preserve_existing_secret(cisco_console_password or "", existing_cisco.get("console_password")),
                ...
                "domain_name": str(cisco_domain_name if cisco_domain_name is not None else existing_cisco.get("domain_name", "lab.local")),
                "management_port": str(cisco_management_port if cisco_management_port is not None else existing_cisco.get("management_port", "GigabitEthernet1/0/1")),
                "management_port_mode": str(cisco_management_port_mode if cisco_management_port_mode is not None else existing_cisco.get("management_port_mode", "do_not_touch")),
                "trusted_console_adapter": (str(cisco_trusted_console_adapter).lower() in {"1", "true", "yes", "on"}) if cisco_trusted_console_adapter is not None else bool(existing_cisco.get("trusted_console_adapter")),
                ...
                "enable_password": preserve_existing_secret(submitted_enable_secret or "", existing_cisco.get("enable_secret") or existing_cisco.get("enable_password")),
            }
        )
    try:
        cfg = runtime["apply_ip_plan"](cfg)
