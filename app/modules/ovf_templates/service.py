...
class OvfTemplateService:
    def register_directory(
        ...
        *,
        ...
        entry = self.template_entry(template_id, summary, template_name=template_name, os_family=os_family)
    ...
    @staticmethod
    def template_entry(template_id: str, summary: dict[str, Any], *, template_name: str = "", os_family: str = "") -> dict[str, Any]:
        return {
            "id": template_id,
    ...
    def templates(self, cfg: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        return sorted([dict(item) for item in values.values()], key=lambda item: str(item.get("name") or item.get("id") or ""))
